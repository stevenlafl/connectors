...
class TestWorkManager:
    def test_init(self, mock_helper: MagicMock, mock_logger: ConnectorLogger):
        wm = WorkManager(mock_helper, mock_logger, "default")
        assert wm._work_name == "default"
    ...
    def test_send_different_name_closes_previous(
        ...
        wm = WorkManager(mock_helper, mock_logger, "default")
        with wm:
            wm.send(["obj1"])  # creates "default" work
    ...
    def test_exit_deletes_work_with_no_bundles(
        ...
        wm = WorkManager(mock_helper, mock_logger, "default")
        with wm:
            wm.send(["obj1"])  # send_bundle called but bundles_sent is empty list
        ...
        wm2 = WorkManager(mock_helper, mock_logger, "default")
        # Manually test: enter, init work, but don't send anything
        wm2.__enter__()
        wm2._current_work = WorkContext("w-empty", mock_helper, mock_logger)
    ...
    def test_close_current_work_no_bundles_deletes(
        ...
        wm = WorkManager(mock_helper, mock_logger, "default")
        wm._current_work = WorkContext("w-1", mock_helper, mock_logger)
    ...
    def test_send_with_explicit_work_name(
        self, mock_helper: MagicMock, mock_logger: ConnectorLogger
    ):
        wm = WorkManager(mock_helper, mock_logger, "default")
        with wm:
            wm.send(["obj"], "custom-work")
        mock_helper.api.work.initiate_work.assert_called_once_with(
            "test-connector-id", "custom-work"
        )
